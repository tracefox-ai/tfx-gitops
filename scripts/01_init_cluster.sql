-- Create Database
CREATE DATABASE IF NOT EXISTS tracefox ON CLUSTER '{cluster}';

-- OTEL LOGS
CREATE TABLE IF NOT EXISTS tracefox.otel_logs_local ON CLUSTER '{cluster}'
(
...
ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/tracefox/otel_logs_local', '{replica}')
...
CREATE TABLE IF NOT EXISTS tracefox.otel_logs ON CLUSTER '{cluster}' AS tracefox.otel_logs_local
ENGINE = Distributed('{cluster}', tracefox, otel_logs_local, rand());

-- OTEL TRACES
CREATE TABLE IF NOT EXISTS tracefox.otel_traces_local ON CLUSTER '{cluster}'
(
    `Timestamp` DateTime64(9) CODEC(Delta(8), ZSTD(1)),
    `TraceId` String CODEC(ZSTD(1)),
    `SpanId` String CODEC(ZSTD(1)),
...
ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/tracefox/otel_traces_local', '{replica}')
PARTITION BY toDate(Timestamp)
ORDER BY (ServiceName, SpanName, toDateTime(Timestamp))
TTL toDate(Timestamp) + toIntervalDay(30)
SETTINGS index_granularity = 8192, ttl_only_drop_parts = 1;

CREATE TABLE IF NOT EXISTS tracefox.otel_traces ON CLUSTER '{cluster}' AS tracefox.otel_traces_local
ENGINE = Distributed('{cluster}', tracefox, otel_traces_local, rand());

-- tracefox SESSIONS
CREATE TABLE IF NOT EXISTS tracefox.tracefox_sessions_local ON CLUSTER '{cluster}'
(
    `Timestamp` DateTime64(9) CODEC(Delta(8), ZSTD(1)),
    `TimestampTime` DateTime DEFAULT toDateTime(Timestamp),
    `TraceId` String CODEC(ZSTD(1)),
...
ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/tracefox/tracefox_sessions_local', '{replica}')
PARTITION BY toDate(TimestampTime)
PRIMARY KEY (ServiceName, TimestampTime)
ORDER BY (ServiceName, TimestampTime, Timestamp)
TTL TimestampTime + toIntervalDay(30)
...
CREATE TABLE IF NOT EXISTS tracefox.tracefox_sessions ON CLUSTER '{cluster}' AS tracefox.tracefox_sessions_local
ENGINE = Distributed('{cluster}', tracefox, tracefox_sessions_local, rand());